...
from quantile_compute import WarmupQuantileAccumulator, EMAUpdate

@dataclass
class RiskBetaDPOConfig:
    """
...
class RiskBetaDPOTrainer(DPOTrainer):
    """
    RiskDPOTrainer with my logic:
      ...
      - update range: uk = (p_hat - tau) / math.sqrt( dleta * (1 - dleta) / n)
      - beta update: beta <- beta * exp(alpha * tanh(gamma * u_k))

    """
    def __init__(self, *args, dynamic_cfg, **kwargs):
        ...
        self._ema: EMAUpdate = None
        self.tau = 0

        # beta dpo
        self.bdpo_M0 = None
        self.bdpo_sigma = None
    ...
    def _maybe_log_margins(self, model_margin: torch.Tensor):
        ...
        if self.dynamic_cfg.jsonl_sample_size and self.dynamic_cfg.jsonl_sample_size > 0:
            # temporarily patch: compute_and_log_model_margin currently writes full sample.
            # We reimplement the jsonl write with truncation but keep the .npy saving.
            import numpy as np
    ...
    def _build_labels_from_prompt(self, input_ids: torch.Tensor, attention_mask: torch.Tensor, prompt_attention_mask: torch.Tensor | None):
        """
    ...
    def compute_loss(self, model, inputs, return_outputs: bool = False, **kwargs):
        ...
        # compute log_prob
        # Case A: your custom collate_fn already provides *_labels
        ...
        # Case B: TRL default pipeline provides prompt_* and chosen/rejected ids but no labels
            ...
            # Some TRL versions may provide prompt_attention_mask with different length than chosen/rejected.
            # If prompt_attn is provided, we mask the first sum(prompt_attn) tokens.
            ...
            if mode != "beta_dpo":
                ...
                if (not self._warmup_done) and (self._warmup_count <= self.dynamic_cfg.warmup_steps):
                    ...
                    if self._warmup_count == self.dynamic_cfg.warmup_steps:
                        ...
                        self._ema = EMAUpdate(tau_0=tau0, q=1.0 - float(self.dynamic_cfg.delta), momentum=float(self.dynamic_cfg.momentum))
                        self.tau = float(tau0)
    ...
    def evaluate(self, eval_dataset=None, **kwargs):
        ...
        if self.args.report_to and "wandb" in self.args.report_to:
            try:
                self.log({
                    "eval/loss": float(metrics["eval_loss"])
                })
            except Exception:
                pass
