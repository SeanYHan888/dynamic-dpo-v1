...
def main():
    ...
    dyn_cfg = RiskBetaDPOConfig(
        ...
        # beta dpo
        mode_loss=str(beta_dpo.get("mode_loss", "risk")),
        bdpo_m=float(beta_dpo["bdpo_m"]),
        bdpo_rho=float(beta_dpo['rho']),
        bdpo_a=float(beta_dpo['alpha']),
        bdpo_min_beta=float(beta_dpo['min_beta']),
        bdpo_eps=float(beta_dpo['eps'])
    )
