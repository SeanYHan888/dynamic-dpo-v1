...
from .rollout import DummyJudge, RolloutGenerator
...
def main() -> None:
    ...
    model_name = config.get("policy_name") or config.get("model_name")
    if not model_name:
    ...
    generator = RolloutGenerator(
        model=model,
        tokenizer=tokenizer,
        num_return_sequences=int(rollout_cfg["k"]),
        **gen_kwargs,
    )
    ...
    meta_base = {
        ...
        "k_candidates": int(rollout_cfg["k"]),
    ...
    def flush(batch: List[dict], out_f) -> bool:
        ...
        for item, cand_list in zip(batch, candidates):
            ...
            if len(cleaned) < 2:
                continue
            best_idx, worst_idx = judge.rank(item["prompt_text"], cleaned)
            record = {
